...
async function getPopularConfigs(): Promise<CachedPopularConfig[]> {
  ...
  // Deduplicate configs that share the same physical board dimensions (edges).
  // Multiple size IDs can map to identical edges (e.g., Kilter HW 7x10 Full Ride vs
  // Mainline vs Auxiliary all have the same edges but different set combos).
  // We group by edges, pick the size_id with the most sets, and aggregate all sets
  // across all size variants. LATERAL JOIN counts climbs per unique edge boundary.
